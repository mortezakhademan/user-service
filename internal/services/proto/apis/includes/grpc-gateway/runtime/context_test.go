...
func TestAnnotateContext_ForwardsGrpcMetadata(t *testing.T) {
	...
	if m, ok := RPCMethod(annotated); !ok {
	...
	if m, ok := HTTPPathPattern(annotated); !ok {
...
func TestAnnotateContext_SupportsTimeouts(t *testing.T) {
	...
	if got, want := time.Until(deadline), DefaultContextTimeout; got-want > acceptableError || got-want < -acceptableError {
...
func TestAnnotateContext_SupportsCustomAnnotators(t *testing.T) {
	...
	annotated, err := AnnotateContext(context.Background(), NewServeMux(WithMetadata(md1), WithMetadata(md2)), request, expectedRPCName)
	if err != nil {
		t.Errorf("runtime.AnnotateContext(ctx, %#v) failed with %v; want success", request, err)
		return
	}
...
func TestAnnotateIncomingContext_SupportsCustomAnnotators(t *testing.T) {
	...
	annotated, err := AnnotateIncomingContext(context.Background(), NewServeMux(WithMetadata(md1), WithMetadata(md2)), request, expectedRPCName)
	if err != nil {
		t.Errorf("runtime.AnnotateIncomingContext(ctx, %#v) failed with %v; want success", request, err)
		return
	}
